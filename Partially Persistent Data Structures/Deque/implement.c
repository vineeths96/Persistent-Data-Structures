...
{
  int_deque_pp* newlist = (int_deque_pp*) malloc(sizeof(int_deque_pp));
...
int_Node* int_update_parentLeft_list_pp(int_deque_pp* list, int_Node* lCrawl, int_Node* newnode, int live_version)
{
...
int_Node* int_update_parentRight_list_pp(int_deque_pp* list, int_Node* lCrawl, int_Node* newnode, int live_version)
{
...
bool int_insert_list_pp(int_deque_pp* list, int data, int index)
{
  if(index<0)
    {
...
int int_delete_list_pp(int_deque_pp* list, int index)
{
  if(index<0)
    {
      printf("Index cannot be negative");
...
{
  if(int_insert_list_pp(deque, data, 0)) return true;
  else false;
}
...
{ 
  if(int_insert_list_pp(deque, data, deque->num_el)) true;
  return true;
}
...
{
  return int_delete_list_pp(deque, 0);
}
  
int int_deque_deleteRear_pp(int_deque_pp* deque)
{
  return int_delete_list_pp(deque, deque->num_el-1);
}
