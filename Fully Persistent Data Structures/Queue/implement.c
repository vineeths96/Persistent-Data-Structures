...
{
  int_queue_fp* newlist = (int_queue_fp*) malloc(sizeof(int_queue_fp));
...
int_Node* int_update_parentLeft_list_fp(int_queue_fp* list, int_Node* lCrawl, int_Node* newnode, int live_version, int version)
{
...
int_Node* int_update_parentRight_list_fp(int_queue_fp* list, int_Node* lCrawl, int_Node* newnode, int live_version, int version)
{
  if(lCrawl == NULL) return NULL;
...
bool int_insert_list_fp(int_queue_fp* list, int data, int index, int version)
{
  if(index<0)
    {
...
bool int_delete_list_fp(int_queue_fp* list, int index, int version)
{
  if(index<0)
    {
      printf("Index cannot be negative");
...
bool int_queue_enqueue_fp(int_queue_fp* queue, int data, int version)
{
  ...
  for(lCrawl = queue->head[version]; lCrawl != NULL; last_index++)
    {
  ...
  return (int_insert_list_fp(queue, data, last_index, version));
}


int int_queue_dequeue_fp(int_queue_fp* queue, int version)
{
  return int_delete_list_fp(queue, 0, version);
}
