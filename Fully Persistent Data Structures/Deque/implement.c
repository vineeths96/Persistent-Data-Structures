...
{
  int_deque_fp* newlist = (int_deque_fp*) malloc(sizeof(int_deque_fp));
...
int_Node* int_update_parentLeft_list_fp(int_deque_fp* list, int_Node* lCrawl, int_Node* newnode, int live_version, int version)
{
...
int_Node* int_update_parentRight_list_fp(int_deque_fp* list, int_Node* lCrawl, int_Node* newnode, int live_version, int version)
{
  if(lCrawl == NULL) return NULL;
...
bool int_insert_list_fp(int_deque_fp* list, int data, int index, int version)
{
  if(index<0)
    {
...
bool int_delete_list_fp(int_deque_fp* list, int index, int version)
{
  if(index<0)
    {
      printf("Index cannot be negative");
...
{
  return (int_insert_list_fp(deque, data, 0, version));
}
...
bool int_deque_insertRear_fp(int_deque_fp* deque, int data, int version)
{
  ...
  for(lCrawl = deque->head[version]; lCrawl != NULL; last_index++)
    {
  ...
  return (int_insert_list_fp(deque, data, last_index, version));
}


int int_deque_deleteFront_fp(int_deque_fp* deque, int version)
{
  return int_delete_list_fp(deque, 0, version);
}
...
{  
  ...
  return int_delete_list_fp(deque, last_index-1, version);
}
